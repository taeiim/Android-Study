...
명령형 프로그래밍은 사용자가 원하는 동작을 하나씩 하나씩 프로그래밍 하는 것입니다. 그렇기 때무넹 매우 구체적이고 자세하고 동작 하나하나의 코드를 작성해야합니다. 이는 '어떤 방법'으로 무언가를 할지를 중심으로 하는 프로그래밍입니다. 	
...
+ 명령형 프로그래밍 <양의 정수와 짝수와 소수 출력>
  ...
  짝수는 GetEvens 함수에서 독립럭으로 구현되어있고 재사용되는 코드가 없다. 어떻게 문제를 해결할지를 구체적으로 기술하고 있습니다.
...
Observable을 데이터베이스 쿼리라고 생각하고 Subscriber은 그 결괄르 가지고 화면에 보여준다고 생각 할 수 있다. 또 다른 예로는 Observable은 화면의 클릭이고 Subscriber은 그에 대한 반응이라고 볼 수 있다. 
